...
// ============================================
// Form Handler - Manipulador de Formulário
// ============================================
const FormHandler = {
  init() {
    this.form = Utils.$('#contact-form');
    this.submitBtn = Utils.$('#submit-btn');
    this.feedback = Utils.$('#form-feedback');
  ...
  validateField(field) {
    ...
    if (field.type === 'email' && field.value) {
      ...
      field.style.borderColor = isValid ? 'rgba(108, 92, 231, 0.3)' : '#ff6b6b';
      return isValid;
    }
    
    field.style.borderColor = 'rgba(108, 92, 231, 0.3)';
    return true;
  },
  ...
  async handleSubmit(e) {
    ...
    this.submitBtn.innerHTML = '<span class="loading-spinner"></span>Enviando...';
    this.feedback.classList.remove('show', 'success', 'error');
    ...
    try {
      ...
      if (response.ok) {
        this.showFeedback('✓ Mensagem enviada com sucesso! Entrarei em contato em breve.', 'success');
        this.form.reset();
      } else {
      ...
      this.showFeedback('✗ Erro ao enviar. Tente novamente ou use o email diretamente.', 'error');
    } finally {
      this.submitBtn.disabled = false;
      this.submitBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Enviar Mensagem';
    }
  },

  showFeedback(message, type) {
    this.feedback.textContent = message;
    this.feedback.classList.add('show', type);
